using FIS.Models;
using FIS.Services;
using FIS.Utils;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.Windows.Forms;

namespace FIS.Forms
{
    public class CustomerBillListPanel : UserControl
    {
        private readonly CustomerBillService _billService = new CustomerBillService();

        private DataGridView _dgv;
        private ComboBox _cboFilter;
        private Label _lblSummary;

        public CustomerBillListPanel()
        {
            this.Dock = DockStyle.Fill;
            this.BackColor = Color.FromArgb(240, 243, 248);
            BuildUI();
        }

        private void BuildUI()
        {
            // ── Header ────────────────────────────────────────────────────────
            var pnlHeader = new Panel
            {
                Dock = DockStyle.Top,
                Height = 64,
                BackColor = Color.FromArgb(26, 55, 100)
            };
            pnlHeader.Controls.Add(new Label
            {
                Text = "Customer Bills",
                Font = new Font("Segoe UI", 15F, FontStyle.Bold),
                ForeColor = Color.White,
                AutoSize = true,
                Location = new Point(20, 12)
            });
            pnlHeader.Controls.Add(new Label
            {
                Text = "Accounts Receivable — track bills and record incoming payments",
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(160, 200, 240),
                AutoSize = true,
                Location = new Point(22, 40)
            });

            // ── Toolbar ───────────────────────────────────────────────────────
            var pnlToolbar = new Panel
            {
                Dock = DockStyle.Top,
                Height = 52,
                BackColor = Color.White
            };
            pnlToolbar.Paint += (s, e) =>
                e.Graphics.DrawLine(
                    new Pen(Color.FromArgb(220, 225, 235), 1),
                    0, 51, pnlToolbar.Width, 51);

            var lblFilter = new Label
            {
                Text = "Filter:",
                Font = new Font("Segoe UI", 9F),
                ForeColor = Color.FromArgb(100, 110, 130),
                AutoSize = true,
                Location = new Point(16, 16)
            };

            _cboFilter = new ComboBox
            {
                Location = new Point(56, 12),
                Size = new Size(150, 26),
                DropDownStyle = ComboBoxStyle.DropDownList,
                Font = new Font("Segoe UI", 9F)
            };
            _cboFilter.Items.AddRange(new object[] {
                "All", "Unpaid", "PartiallyPaid", "Overdue", "Paid" });
            _cboFilter.SelectedIndex = 0;
            _cboFilter.SelectedIndexChanged += (s, e) => LoadData();

            var btnGenerateBills = MakeToolbarButton("Generate Bills", Color.FromArgb(30, 120, 60));
            btnGenerateBills.Location = new Point(226, 10);
            btnGenerateBills.Size = new Size(140, 32);
            btnGenerateBills.Click += BtnGenerateBills_Click;

            var btnMarkOverdue = MakeToolbarButton("Mark Overdue", Color.FromArgb(160, 60, 50));
            btnMarkOverdue.Location = new Point(376, 10);
            btnMarkOverdue.Click += BtnMarkOverdue_Click;

            var btnRefresh = MakeToolbarButton("↻ Refresh", Color.FromArgb(80, 100, 140));
            btnRefresh.Location = new Point(510, 10);
            btnRefresh.Click += (s, e) => LoadData();

            _lblSummary = new Label
            {
                Text = "",
                Font = new Font("Segoe UI", 8.5F),
                ForeColor = Color.FromArgb(120, 130, 150),
                AutoSize = true,
                Location = new Point(660, 18)
            };

            pnlToolbar.Controls.AddRange(new Control[] {
                lblFilter, _cboFilter,
                btnGenerateBills, btnMarkOverdue, btnRefresh,
                _lblSummary });

            // ── DataGridView ──────────────────────────────────────────────────
            _dgv = new DataGridView
            {
                Dock = DockStyle.Fill,
                ReadOnly = true,
                AllowUserToAddRows = false,
                AllowUserToDeleteRows = false,
                AllowUserToResizeRows = false,
                BorderStyle = BorderStyle.None,
                BackgroundColor = Color.FromArgb(240, 243, 248),
                GridColor = Color.FromArgb(220, 225, 235),
                RowHeadersVisible = false,
                AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill,
                SelectionMode = DataGridViewSelectionMode.FullRowSelect,
                MultiSelect = false,
                Font = new Font("Segoe UI", 9F),
                CellBorderStyle = DataGridViewCellBorderStyle.SingleHorizontal,
                ColumnHeadersBorderStyle = DataGridViewHeaderBorderStyle.Single
            };
            _dgv.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(245, 247, 252);
            _dgv.ColumnHeadersDefaultCellStyle.ForeColor = Color.FromArgb(80, 95, 120);
            _dgv.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 8.5F, FontStyle.Bold);
            _dgv.ColumnHeadersHeight = 36;
            _dgv.ColumnHeadersHeightSizeMode = DataGridViewColumnHeadersHeightSizeMode.DisableResizing;
            _dgv.RowTemplate.Height = 32;
            _dgv.DefaultCellStyle.SelectionBackColor = Color.FromArgb(210, 225, 250);
            _dgv.DefaultCellStyle.SelectionForeColor = Color.FromArgb(20, 40, 80);
            _dgv.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(248, 249, 252);
            _dgv.CellDoubleClick += Dgv_CellDoubleClick;

            AddColumn("CustomerBillId", "ID", 30, false);
            AddColumn("CustomerName", "Customer", 160, true);
            AddColumn("BillDate", "Bill Date", 100, true);
            AddColumn("DueDate", "Due Date", 100, true);
            AddColumn("TotalAmountDue", "Total", 90, true);
            AddColumn("BalanceRemaining", "Balance", 90, true);
            AddColumn("AmountPaid", "Paid", 90, true);
            AddColumn("Status", "Status", 100, true);
            _dgv.Columns["CustomerBillId"].Visible = false;

            var pnlGrid = new Panel
            {
                Dock = DockStyle.Fill,
                Padding = new Padding(16, 12, 16, 16),
                BackColor = Color.FromArgb(240, 243, 248)
            };
            pnlGrid.Controls.Add(_dgv);

            this.Controls.Add(pnlGrid);
            this.Controls.Add(pnlToolbar);
            this.Controls.Add(pnlHeader);

            LoadData();
        }

        // ── Load data ─────────────────────────────────────────────────────────
        private void LoadData()
        {
            try
            {
                List<CustomerBill> bills = _billService.GetAllBills();

                string filter = _cboFilter.SelectedItem?.ToString() ?? "All";
                if (filter != "All")
                    bills = bills.FindAll(b => b.Status == filter);

                _dgv.Rows.Clear();

                decimal totalDue = 0;
                decimal totalRemaining = 0;

                foreach (var b in bills)
                {
                    decimal amountPaid = b.TotalAmountDue - b.BalanceRemaining;

                    int rowIdx = _dgv.Rows.Add(
                        b.CustomerBillId,
                        GetCustomerName(b),
                        b.BillDate.ToString("MMM dd, yyyy"),
                        b.DueDate.ToString("MMM dd, yyyy"),
                        b.TotalAmountDue.ToString("C"),
                        b.BalanceRemaining.ToString("C"),
                        amountPaid.ToString("C"),
                        b.Status);

                    // Color row by status
                    ApplyBillColor(_dgv.Rows[rowIdx], b.Status);

                    totalDue += b.TotalAmountDue;
                    totalRemaining += b.BalanceRemaining;
                }

                _lblSummary.Text =
                    $"{bills.Count} bill(s)   " +
                    $"Total Billed: {totalDue:C}   " +
                    $"Outstanding: {totalRemaining:C}   " +
                    $"Collected: {(totalDue - totalRemaining):C}";
            }
            catch (Exception ex)
            {
                MessageHelper.ShowError("Could not load bills.\nDetails: " + ex.Message);
            }
        }

        // ── Row color by status ───────────────────────────────────────────────
        // CustomerBill has 4 statuses — more granular than invoices so we
        // apply colors directly here rather than using StatusColors utility.
        private void ApplyBillColor(DataGridViewRow row, string status)
        {
            if (status == "Overdue")
                row.DefaultCellStyle.BackColor = Color.FromArgb(255, 204, 204);
            else if (status == "PartiallyPaid")
                row.DefaultCellStyle.BackColor = Color.FromArgb(255, 255, 204);
            else if (status == "Paid")
                row.DefaultCellStyle.BackColor = Color.FromArgb(204, 255, 204);
            else
                row.DefaultCellStyle.BackColor = Color.White;
        }

        // ── CustomerName from bill — service join gives us CustomerName ───────
        // The mapper doesn't populate the Customer navigation property,
        // but the SQL JOIN returns CustomerName as a flat column stored
        // in the DataRow. We re-query via GetById to get it safely.
        // For the list we just use the bill object directly — CustomerName
        // isn't on the model, so we show CustomerId as fallback.
        private string GetCustomerName(CustomerBill b)
        {
            // CustomerBillService.GetAllBills() JOINs Customers and returns
            // CustomerName in the DataRow, but MapRowToCustomerBill doesn't
            // store it on the model (no CustomerName field on CustomerBill).
            // We show CustomerId here; the detail panel shows the full name.
            return $"Customer #{b.CustomerId}";
        }

        // ── Toolbar button handlers ───────────────────────────────────────────
        private void BtnGenerateBills_Click(object sender, EventArgs e)
        {
            if (!MessageHelper.Confirm(
                "Generate bills for all shipped orders that haven't been billed yet?",
                "Generate Bills"))
                return;

            int count = _billService.GeneratePendingBills();
            MessageHelper.ShowSuccess($"{count} bill(s) generated.");
            LoadData();
        }

        private void BtnMarkOverdue_Click(object sender, EventArgs e)
        {
            if (!MessageHelper.Confirm(
                "Mark all unpaid bills past their due date as Overdue?",
                "Mark Overdue"))
                return;

            int count = _billService.MarkOverdueBills();
            MessageHelper.ShowSuccess($"{count} bill(s) marked as Overdue.");
            LoadData();
        }

        // ── Double-click → open bill detail with payment option ───────────────
        private void Dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0) return;
            int billId = Convert.ToInt32(
                _dgv.Rows[e.RowIndex].Cells["CustomerBillId"].Value);

            var detail = new CustomerBillDetailPanel(billId);
            detail.OnSaved += () => LoadData();
            SwapToDetail(detail);
        }

        private void SwapToDetail(UserControl panel)
        {
            var dashboard = this.FindForm() as MainDashboard;
            if (dashboard == null) return;
            panel.Dock = DockStyle.Fill;
            dashboard.pnlContent.Controls.Clear();
            dashboard.pnlContent.Controls.Add(panel);
        }

        private void AddColumn(string name, string header, int fillWeight, bool visible)
        {
            _dgv.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = name,
                HeaderText = header,
                FillWeight = fillWeight,
                Visible = visible,
                SortMode = DataGridViewColumnSortMode.Automatic
            });
        }

        private Button MakeToolbarButton(string text, Color backColor)
        {
            var btn = new Button
            {
                Text = text,
                Size = new Size(125, 32),
                BackColor = backColor,
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 8.5F, FontStyle.Bold),
                Cursor = Cursors.Hand
            };
            btn.FlatAppearance.BorderSize = 0;
            return btn;
        }
    }
}